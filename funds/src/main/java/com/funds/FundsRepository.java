...
//	@Query(nativeQuery = true, value = "SELECT * FROM funds where fund_amc= ?;")
	...
	@Query(nativeQuery = true, value = "SELECT fund_name, fund_amc, fund_risk, fund_aum, fund_type, fund_nav, fund_mgr, fund_desc, img_src,fh1month, fh1year, fh_total FROM funds f join fund_history h on f.fund_id = h.fund_id where f.fund_id=?;")
